...
async def sync_config(uc, bc):
    ...
    try:
        ...
        if lottery_init.get("code") == 1 and lottery_init.get("data"):
            ...
            n_series = len(body.get('lottery_series', []))
            n_games = len(body.get('lottery_games', []))
            print(f"  lottery: {n_series} series, {n_games} games")
    except Exception as e:
...
async def sync_members(uc, bc):
    ...
    checked, found = verify.get("checked", 0), verify.get("found", 0)
    print(f"  verify: {status} ({checked} checked, {found} found)")
    ...
    try:
        async with httpx.AsyncClient(base_url=BACKEND_BASE) as client:
            ...
            for ep in resp.json().get("endpoints", []):
                ...
                name = ep["endpoint"]
                count = ep.get("last_sync_count")
                sync_st = ep.get("sync_status")
                print(f"  {name:<22} last_data: {last_date}  count: {count}  status: {sync_st}")
    except Exception as e:
