...
from src.hubserver.core.exceptions.http_exceptions import DuplicateValueException, ForbiddenException, NotFoundException
from src.hubserver.features.user.router import erase_user, patch_user, read_user, read_users, write_user
from src.hubserver.features.user.schema import UserCreate, UserRead, UserUpdate

_PATCH_BASE = "src.hubserver.features.user.router"
...
class TestReadUsers:
    ...
    async def test_read_users_success(self, mock_db):
        ...
        with patch(f"{_PATCH_BASE}.crud_users") as mock_crud:
            ...
            with patch(f"{_PATCH_BASE}.paginated_response") as mock_paginated:
...
class TestEraseUser:
    ...
    async def test_erase_user_success(self, mock_db, current_user_dict, sample_user_read):
        ...
        with patch(f"{_PATCH_BASE}.crud_users") as mock_crud:
            ...
            with patch(f"{_PATCH_BASE}.blacklist_token", new_callable=AsyncMock) as mock_blacklist:
